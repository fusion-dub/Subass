...
local attach_to_video_h = true          -- прив'язувати по висоті до відеовікна
local attach_offset = 0                 -- відступ у відсотках (0-100)
local attach_manual_x = 0               -- ручна корекція X (пікселі)
local attach_manual_y = 42              -- ручна корекція Y (пікселі)
...
local assimilation_rules = {
    ...
    {"сч", "щ"},               -- сч → щ [шч]: козащина; Борищенко
    {"тч", "чч"},              -- вотчина → воччина  [т+ч → ч:]
    {"тш", "чш"},              -- багатший → багачший  [т+ш → чш]
    {"тц", "цц"},              -- коритце → корицце, братця → брацця  [т+ц → ц:]
    {"дч", "чч"},              -- відчепити → вич**ч**епити  (д→т→ч)
    {"дш", "чш"},              -- підшити → піч**ш**ити  [д→т→ч перед ш]
    {"дц", "дзц"},             -- відцідити → відзцідити  [д→дз перед ц]
...
local function apply_assimilation_recursive(text, offset)
    ...
    for _, r in ipairs(assimilation_rules) do
        ...
        if p then
            ...
            if not skip and (not best_pos or p < best_pos) then
                best_pos = p
                best_rule = r
            end
        end
...
local function load_settings()
    ...
    border = (reaper.GetExtState(SETTINGS_SECTION, "border") ~= "false")
    ...
    attach_manual_y = tonumber(reaper.GetExtState(SETTINGS_SECTION, "attach_manual_y")) or 42
...
local function sync_external_data()
    ...
    local new_lines = {}
    local l_dump = ""
    local ok_c, count_str = reaper.GetProjExtState(0, "Subass_Notes", "ass_lines_count")
    
    if ok_c and count_str ~= "" then
        local count = tonumber(count_str) or 0
        for i = 1, count do
            local ok_x, chunk = reaper.GetProjExtState(0, "Subass_Notes", "ass_lines_chunk_" .. i)
            if ok_x then l_dump = l_dump .. chunk end
        end
    else
        -- Fallback to legacy single-key format
        local ok_l, legacy_dump = reaper.GetProjExtState(0, "Subass_Notes", "ass_lines")
        if ok_l then l_dump = legacy_dump end
    end
...
local function check_video_window_moved()
    ...
    if is_mac and not is_points then
...
local function loop()
    ...
    local continue_running = (win_open ~= false) and not close_requested
